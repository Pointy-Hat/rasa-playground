...
def read_intent_report(path):
    ...
    jsonl = [{**v, 'config': path.split("/")[0]} for k,v in blob.items() if 'weighted avg' in k]
...
st.cache()
def read_pandas():
    dirs = [str(d) for d in os.listdir(".") if os.path.isdir(d)]
    paths = [os.path.join(d, "intent_report.json") for d in dirs if os.path.exists((os.path.join(d,"intent_report.json")))]
    configurations = set([p.split("/")[0] for p in paths])
    intent_df = pd.concat([read_intent_report(p) for p in paths if 'intent_report' in str(p)])
    intent_df.pipe(add_zeros, all_configs=configurations)
    ...
    #entity_df = pd.concat([read_entity_report(p) for p in paths])
    return intent_df#, entity_df

intent_df = read_pandas()
